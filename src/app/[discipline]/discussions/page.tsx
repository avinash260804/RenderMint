...
export default async function DisciplineDiscussionsPage({ params }: DisciplinePageProps) {
  const { discipline: disciplineSlug } = await params;
  const { discipline, posts } = await getDisciplineFeed(disciplineSlug, "discussion");
