...
export function getPostsByDiscipline(slug: DisciplineSlug) {
  return communityPosts.filter((post) => post.discipline === slug);
}

export function getPostsByType(type: CommunityPostType, slug?: DisciplineSlug) {
  return communityPosts.filter((post) => post.type === type && (!slug || post.discipline === slug));
}

export function getHomeSections() {
  return {
    trendingDiscussions: getPostsByType("discussion"),
    critiqueRequests: getPostsByType("critique"),
    featuredShowcases: getPostsByType("showcase"),
    solvedHelp: communityPosts.filter((post) => post.type === "help" && post.solved),
    weeklyResources: getPostsByType("resource"),
  };
}
